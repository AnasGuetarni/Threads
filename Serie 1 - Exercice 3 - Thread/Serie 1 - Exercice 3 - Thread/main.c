...
void *thread(void *RES) { // On crée une procédure qui pointe sur l'id d'un thread
    int resultat = *((int *) RES); // On initialise l'id du thread
...
int main(int argc, const char * argv[]) {
    ...
    int code = pthread_create(&threads, NULL, thread, &RES);
